...
def _bind_root(vault: str) -> Path:
    ...
    from silica_core.kernel.vault_manifest import apply_manifest_to_config
...
def _parser() -> argparse.ArgumentParser:
    from silica_core._version import __version__
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not a.cmd:
        ...
        return 0
    if a.cmd == "update":
        from silica_core.update import update as run_update
    ...
    if a.cmd == "repl":
        from silica_core.repl import main as repl_main
        return repl_main()
    if a.cmd == "mcp":
        import logging

        from silica_core.ui.mcp import run_mcp
        logging.basicConfig(stream=sys.stderr, level=logging.WARNING)
        return run_mcp(extended=a.extended, retrieval=a.retrieval)
    if a.cmd == "doctor":
        from silica_core.config import CONFIG
        from silica_core.onboarding import checks
    ...
    if a.cmd == "setup":
        from silica_core.onboarding.setup_client import run_setup
    ...
    if a.cmd == "connect":
        from silica_core.ui.connect import run_connect
        return run_connect()
    if a.cmd == "import":
