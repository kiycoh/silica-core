...
CLIENTS: dict[str, Spec] = {
    ...
    "cursor": Spec("Cursor", "json", _home(".cursor", "mcp.json"), ("mcpServers",), entry=_stdio,
                   note="for the agent rule too: copy silica_core/skills/silica/SKILL.md "
    ...
    "windsurf": Spec("Windsurf", "json", _home(".codeium", "windsurf", "mcp_config.json"),
                     ...
                     note="for the agent rule too: copy silica_core/skills/silica/SKILL.md to .windsurfrules"),
...
RECIPES = {
    ...
    "python": (
        ...
        "  from silica_core.core import search, read   # same payloads, no subprocess",
    ),
    "generic": (
        "Any other MCP client (Void, Jan, Factory Droid, Antigravity) —",
        "paste this into its server registry:",
        '  {"command": "uvx", "args": ["--from", "silica-core[mcp,dense]", "silica", "mcp", "--retrieval", "local-hybrid"]}',
        "Harnesses that also read skills: copy silica_core/skills/silica/SKILL.md into their",
...
def skill_path() -> Path:
    ...
    return Path(str(files("silica_core") / "skills" / "silica" / "SKILL.md"))
